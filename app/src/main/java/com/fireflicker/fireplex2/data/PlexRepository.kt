...
class PlexRepository(private val context: Context) {
    ...
    suspend fun vlcPlayerSettings(): VlcPlayerSettings {
        val raw = context.dataStore.data.first()[vlcPlayerSettingsKey]
        return if (raw.isNullOrBlank()) {
            VlcPlayerSettings()
        } else {
            runCatching { json.decodeFromString(VlcPlayerSettings.serializer(), raw) }
                .getOrDefault(VlcPlayerSettings())
        }
    }

    suspend fun saveVlcPlayerSettings(settings: VlcPlayerSettings) {
        val encoded = json.encodeToString(VlcPlayerSettings.serializer(), settings)
        context.dataStore.edit { prefs ->
            prefs[vlcPlayerSettingsKey] = encoded
        }
    }

    suspend fun mpvPlayerSettings(): MpvPlayerSettings {
        val raw = context.dataStore.data.first()[mpvPlayerSettingsKey]
        return if (raw.isNullOrBlank()) {
            MpvPlayerSettings()
        } else {
            runCatching { json.decodeFromString(MpvPlayerSettings.serializer(), raw) }
                .getOrDefault(MpvPlayerSettings())
        }
    }

    suspend fun saveMpvPlayerSettings(settings: MpvPlayerSettings) {
        val encoded = json.encodeToString(MpvPlayerSettings.serializer(), settings)
        context.dataStore.edit { prefs ->
            prefs[mpvPlayerSettingsKey] = encoded
        }
    }
    ...
    suspend fun imageUrl(path: String, width: Int = 300, height: Int = 450): String = withContext(Dispatchers.IO) {
        ...
        if (path.isBlank()) {
            ...
            val safeWidth = width.coerceIn(120, 1920)
            val safeHeight = height.coerceIn(120, 1080)
            val encodedPath = encode(path)

            "${base.trimEnd('/')}/photo/:/transcode" +
                "?width=$safeWidth" +
                "&height=$safeHeight" +
                "&minSize=1" +
                "&upscale=1" +

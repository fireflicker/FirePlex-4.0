...
class PlexRepository(private val context: Context) {
    ...
    suspend fun libraryItems(library: PlexLibrary): List<PlexMediaItem> = withContext(Dispatchers.IO) {
        ...
        while (start < 30000) {
            ...
            val pageUrl = "${baseUrl}${joiner}X-Plex-Container-Start=$start&X-Plex-Container-Size=$pageSize"
            val pageItems = PlexXmlParser.parseMediaItems(serverRequest(pageUrl, token))
        ...
        allItems
            ...
            .sortedWith(compareBy<PlexMediaItem> { it.title.lowercase() }.thenByDescending { it.addedAt })
    }
